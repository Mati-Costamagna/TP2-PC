...
public class Logger extends Thread {
    ...
    private final BlockingQueue<String> transiciones = new LinkedBlockingQueue<>(); // Cola para mantener los mensajes
                                                                                    // de registro de transiciones
                                                                                    // entrantes
    ...
    public boolean alcanzoCantMaxInvariantes() {
        // La condición de parada considera la suma de los conteos de todos los
        // invariantes.
    ...
    @Override
    public void run() {
        // El tamaño del buffer debe ser lo suficientemente grande para que entre la
        // secuencia más larga que estamos verificando.
        // La secuencia más larga es complejidadAlta con una longitud de 7.
        LinkedList<String> buffer = new LinkedList<>(); // Búfer de ventana deslizante para verificaciones de
                                                        // invariantes
        try {
            // Repetir mientras no se haya alcanzado el conteo máximo de invariantes Y
            // (no se haya indicado explícitamente detenerse O todavía haya elementos en la
            // cola para procesar).
            while (!alcanzoCantMaxInvariantes() && (!finalizar.get() || !transiciones.isEmpty())) {
                String transicion;
                try {
                    if (finalizar.get()) {
                        // Si se está deteniendo, usar poll() para recuperar elementos existentes sin
                        // bloqueo
                    ...
                    // Si se interrumpe, generalmente es una señal para detenerse. Establecer
                    // bandera y reinterrumpir.
                    finalizar.set(true);
                    e.printStackTrace();
                    Thread.currentThread().interrupt(); // Restaurar el estado de interrupción
                    continue; // Reevaluar la condición del bucle para vaciar los elementos restantes de la
                              // cola
                }
                ...
                if (buffer.size() >= complejidadSimple.size()) {
                    if (buffer.subList(buffer.size() - complejidadSimple.size(), buffer.size())
                            .equals(complejidadSimple)) {
                ...
                if (buffer.size() >= complejidadMedia.size()) {
                    if (buffer.subList(buffer.size() - complejidadMedia.size(), buffer.size())
                            .equals(complejidadMedia)) {
            ...
            // Escribir los conteos finales de solo las secuencias deseadas en el archivo y
            // cerrar el logger
            try {
